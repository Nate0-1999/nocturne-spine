...
3. PACKAGING. Two wheels, zero clones: `nocturne` (harness repo — daemon,
   ...
   V0.1 POSITIONING (owner ruling, packaging review 2026-08-01; v2.35):
   `nocturne up` — the local Postgres/pgvector palace, same spine, same
   schema, same scorer — is THE PUBLIC PATH. `nocturne deploy` is
   OWNER/INTERNAL: it encodes the owner's own foundation (project,
   region, instance) and is not general GCP onboarding; presenting it as
   such would be untruthful. Generic per-install cloud provisioning,
   database-fleet lifecycle (expand/contract migration discipline,
   advisory deploy locks, per-release migration matrices, managed
   backup/restore for strangers) is a DELIBERATE FUTURE PRODUCT decision
   — "we operate no one's database but our own until we choose to be a
   SaaS." The Palace identity law is UNCHANGED: the owner's Palace
   remains the always-on cloud heart (the founding local-only rejection
   was about the OWNER's multi-machine continuity, which stands); a trial
   user's local palace is a complete, honest product on the same HTTP
   boundary, and moving one later is explicit export/import, never
